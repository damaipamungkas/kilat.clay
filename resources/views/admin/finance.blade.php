...
<body data-theme="">
    ...
    <script>
        ...
        function applyAppTheme() {
            // Mengambil data tema dari localStorage dengan berbagai kemungkinan kunci key
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Observer untuk memantau perubahan teks role agar tidak berubah ke 'coach'
        ...
        // --- HELPER UNTUK MATA UANG & PERIODE ---
        ...
        // --- PERBAIKAN PENGAMBILAN & PENGUNCIAN ROLE AKTIF ---
        function getActiveRole() {
            ...
            if (!activeRole) {
                let sidebarUserEl = document.querySelector('.sidebar-user, .user-profile, [class*="user"]');
                if (sidebarUserEl && sidebarUserEl.innerText) {
                    activeRole = sidebarUserEl.innerText.trim();
                }
            }

            if (!activeRole || activeRole.toLowerCase() === 'coach') {
        ...
        function getValidAdminList() {
            let registeredUsers = JSON.parse(localStorage.getItem('manageUsersData')) ||
            ...
            registeredUsers.forEach(u => {
                ...
                if (roleStr === 'ADMIN' || roleStr.includes('ADMIN') || roleStr.includes('MASTER')) {
                    ...
                    if (nameVal) adminSet.add(nameVal);
                }
            });

            if (adminSet.size === 0) {
                adminSet.add("admin bendahara");
                adminSet.add("admin demo 1");
                adminSet.add("Master Admin System");
            }
            ...
            ['bulanan', 'harian', 'daftar', 'lain', 'keluar'].forEach(cat => {
                if (financeDB[cat]) {
                    financeDB[cat].forEach(item => {
                        let currentAcc = (item.account || '').toLowerCase();
                        if (currentAcc.includes('parent') || currentAcc.includes('atlet') || (!validAdmins.includes(item.account) && !currentAcc.includes('admin') && !currentAcc.includes('master'))) {
        ...
        function renderTableHarianCustom() {
            ...
            filteredItems.forEach((item) => {
                ...
                tr.innerHTML = `
                    ...
                    <td><span class="badge-account"><i class="fa-solid fa-user-shield"></i> ${item.account || 'Admin'}</span></td>
                    <td>
